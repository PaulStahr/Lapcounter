...
std::istream & operator >> (std::istream & input, options_t & opt)
{
    ...
    while (getline (input,line))
    {
        ...
        if (seperator != line.end())
        {
            ...
            if (name == opt_string::led_stripe_white)
            {
                opt._led_stripe_white = std::strtol(value.c_str(), nullptr, 16);
            }
            else if (name == opt_string::color_error)
            {
                opt._color_error = std::strtol(value.c_str(), nullptr, 16);
            }
            else if (name == opt_string::color_foreground)
            {
                opt._color_foreground = std::strtol(value.c_str(), nullptr, 16);
            }
            else if (name == opt_string::color_background)
            {
                opt._color_background = std::strtol(value.c_str(), nullptr, 16);
            }
            else if (name == opt_string::color_selection)
            {
                opt._color_selection = std::strtol(value.c_str(), nullptr, 16);
            }
            else
            {
